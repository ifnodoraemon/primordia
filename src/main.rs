...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 1. 创世：向虚空中注入两个无预设规则的原初存在
    println!("--- [步骤 1 / Step 1: 创世凝结原初实体 / Entity Genesis] ---");
    let stone_id = world.add_entity(
    ...
    // 3. 碰撞与化生测试
    println!("\n--- [步骤 3 / Step 3: 两实体发生碰撞与相变化生 / Collision & Morphogenesis] ---");
    world.collide(&stone_id, &fire_id).await?;

    // 4. 世界推进一个自演化纪元
    println!("\n--- [步骤 4 / Step 4: 世界推进自生长周期 / Autonomous World Evolution Tick] ---");
    world.tick().await?;
